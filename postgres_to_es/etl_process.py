...
class EtlProcess:
    ...
    def init_process(self, pg_conn: _connection, curs: DictCursor, es_connect: dict, state: State, model_name: str) -> tuple:
    ...
    def check_and_update(self, last_modified, postgres_extractor, es_loader, state: State, iter_model_name: str) -> None:
        while True:
            ...
            if data:
                ...
                state.set_state('last_modified', data[-1]['modified'].strftime('%Y-%m-%d') if 'modified' in data[-1] else datetime.now().strftime('%Y-%m-%d'))
            else:
